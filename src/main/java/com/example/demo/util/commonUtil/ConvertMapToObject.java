...
public class ConvertMapToObject {
    public static List<Map<String, Object>> convertData(List<List<Object>> dataList,int start) {
        List<Map<String, Object>> resultMapList = new ArrayList<>();

        if (dataList != null && !dataList.isEmpty()) {
            List<Object> keyList = dataList.get(start);
            int keySize = keyList.size();

            for (int i = start+1; i < dataList.size(); i++) {
    ...
    public static <T> List<T> convertMapToObject(List<Map<String, Object>> list, Class<T> clazz,boolean flag) {
        ...
        try {
            for (Map<String, Object> map : list) {
                ...
                for (Map.Entry<String, Object> entry : map.entrySet()) {
                    ...
                    String columnName;
                    if(flag){
                        columnName = convertToCamelCase(fieldName);
                    }else {
